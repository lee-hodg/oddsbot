...
class Market(object):
    """A market."""
    def __init__(self, name, myid, pid, inrunning, parentEventId,
                 startTime, selections=None, **kwargs):
        ...
        self.id = myid
        # parent id (this is league)
        self.pid = pid
        # is the market 'in running?'
        self.inrunning = inrunning
        # event classifier id (so can link markets back to parent event)
        self.parentEventId = parentEventId
        # startTime
        self.startTime = startTime

        # Any selections for the market (if called with GetSubTreeWithSelections
        # for e.g.)
        if selections:
...
class HollowSelection(object):
    """Type of selection returned from GetSubTreeWithSelections"""
    def __init__(self, name, sid, src, **kwargs):

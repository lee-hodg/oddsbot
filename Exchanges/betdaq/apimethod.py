...
class ApiGetEventSubTreeWithSelections(ApiMethod):
    ...
    def call(self, ids):
        self.req.EventClassifierIds = ids
        self.req._WantPlayMarkets = False
        apilog.info('calling BDAQ Api GetEventSubTreeWithSelections')
        response = self.client.service.\
                 GetEventSubTreeWithSelections(self.req)
        # I amended parse function to grab selections too
        allmarkets_with_sels = apiparse.ParseGetEventSubTreeWithSelections(response)
        return allmarkets_with_sels
